...
# ========== CONFIGURATION ==========
IMAGES_DIR = "images_affirmations"      # separate folder from third channel
OUTPUT_VIDEO = "affirmations_video.mp4"
ASSIGNMENT_DURATION = 5
OUTRO_DURATION = 10
INTRO_DURATION = 5
VIDEO_SIZE = (1080, 1920)
MUSIC_FILE = "background_music.mp3"      # same music file or different
...
def main():
    print("="*60)
    print("🎬 Affirmations – YouTube Shorts Generator")
    print("="*60)
    ...
    if est_duration > 60:
        print(f"⚠️ Duration {est_duration}s > 60s, adjust MAX_AFFIRMATIONS or durations.")
        sys.exit(1)
    ...
    title = f"{intro_title} | {today} | Daily Affirmations"
    if len(title) > 100:
        title = title[:97] + "..."

    aff_list = "\n".join([f"{i+1}. {a}" for i, a in enumerate(affirmations)])
    description = f"""Start your day with these {len(affirmations)} powerful affirmations.
