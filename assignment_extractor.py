#!/usr/bin/env python3
"""
Creative Daily - Assignment Extractor
...
In this video, discover how to Secure a Creative Entrepreneur Habits by using simple Daily Assignments in Dubai. Do these powerful assignments every time you visit Burj Khalifa, Dubai Mall, or a Dubai Restaurant—even while doing Dubai Tourism. Engange nature with a Creative Royalty Tracking Habits, and train your brain to default toward building a Passive Wealth System.

Whether you're out enjoying the city or working on your business, let these assignments align you with success, abundance, and creativity. Watch now and start acting like a true creative entrepreneur in Dubai."""
...
def extract_assignment_from_text(page_text: str) -> str:
    ...
    # Find the exact position of "Assignment" in the text
    ...
    # Look for stop markers after the assignment
    ...
    # Extract text between Assignment and stop marker
    ...
    for line in lines:
        ...
        # Skip the line that contains "Assignment" itself
        ...
        if line_stripped == '' and len(assignment_lines) > 0:
            # Check if next non-empty line is a stop marker
            continue
        
        # Keep the line if it's not empty and not a stop marker
        if line_stripped and not line_stripped.isdigit():
            # Remove any "Good luck" or similar
...
def extract_affirmation_from_text(page_text: str) -> str:
    """Extract affirmation (for reference, not used in assignment script)"""
    pass
...
def create_assignment_image_from_template(assignment_text: str, target_date: str, output_path: str = None, template_path: str = TEMPLATE_PATH) -> str:
    """
    Uses a Canva template as background and overlays the assignment text and date.
    """
    
    if output_path is None:
        output_path = f"assignment_{target_date}.png"
    ...
    try:
        ...
        # Label "DAILY ASSIGNMENT" position
        label_x = img.width // 2  # Center
        label_y = 150
        ...
        text_y = 350      # Top Y position for assignment text
        line_height = 65  # Space between lines
        
        # Draw "DAILY ASSIGNMENT" label
        ...
        # Wrap assignment text to fit within the defined width
        # For font size 48, ~45-50 characters per line on 1920px width
        chars_per_line = 50
        wrapped_text = textwrap.wrap(assignment_text, width=chars_per_line)
        print(f"   📝 Wrapped assignment into {len(wrapped_text)} lines")
        
        # Draw assignment text with bullet points or numbered list formatting
        current_y = text_y
        
        for i, line in enumerate(wrapped_text):
            # Add bullet point or number for list items
            if line.strip().startswith(('1', '2', '3', '4', '5', '•', '-', '*')):
                # Keep original formatting
                display_line = line
            else:
                # Add a checkmark bullet for action items
                display_line = f"✓ {line}"
...
def create_assignment_video(image_path: str, output_path: str = None, slide_duration: int = 30, audio_file: str = None) -> str:
    """Create 30-second video with the assignment image"""
    ...
    try:
        # Use the image directly (template has all design elements)
...
class AssignmentExtractor:
    ...
    def get_assignment_from_page(self, page_info: dict) -> str:
        """Extract ONLY assignment from page text"""
    ...
    def upload_to_youtube(self, video_path: str, target_date: str, assignment_text: str) -> dict:
        ...
        full_title = f"Secure Creative Entreprenuer Mindset! Do This, | {formatted_date} | Stupid Orange Dubai"
        
        video_description = f"""📋 Secure Creative Entreprenuer Habits!, Do This, DAILY ASSIGNMENT - {formatted_date} 📋Are you ready for a Dubai Business Setup or have you already started your journey? Whether you're in the Dubai Fashion Business, enjoy Dubai Shopping, or simply keep up with Dubai news, cultivating the right Entrepreneur habits is everything.
...
Whether you're out enjoying the city or working on your business, let these habits align you with success, abundance, and creativity. Watch now and start acting like a true creative entrepreneur in Dubai.
...
"""
        ...
        try:
            ...
            body = {
                'snippet': {
                    ...
                    'tags': ['assignment', 'dailyassignment', 'creativedaily', 'stupidestbrokeguy', 'action'],
                    'categoryId': '22'
                },
    ...
    def process_date(self, target_date: str, post_to_youtube: bool = True, audio_file: str = None) -> dict:
        print("="*60)
        print("📋 CREATIVE DAILY - ASSIGNMENT EXTRACTOR (30 sec)")
        print("="*60)
        ...
        # Create assignment image using Canva template
        image_path = create_assignment_image_from_template(
            assignment, 
            target_date, 
            os.path.join(self.output_dir, f"assignment_{target_date}.png")
        )
...
if __name__ == "__main__":
    print("="*60)
    print("📋 ASSIGNMENT EXTRACTOR - WITH CANVA TEMPLATE")
    print("="*60)
