...
- AUTO THUMBNAIL from image content at t=1 second (no yellow background, stretched to fill)
"""
...
def get_image_position_at_time(t: float, slide_duration: int = 18, screen_height: int = 1080, new_height: int = None) -> float:
    """
    Calculate the Y position of the image at a specific time t.
    This mirrors the animation logic from create_sliding_animation_video.
    """
    if new_height is None:
        new_height = 2000  # Default fallback
    
    # Original animation parameters (from working code)
    ...
    # Calculate position at requested time
...
def extract_thumbnail_from_video(video_path: str, output_path: str = None, time_seconds: float = THUMBNAIL_CAPTURE_TIME) -> str:
    """
    Extract thumbnail from video at specified time - cropping to JUST the image content (no yellow background)
    Then stretch the content to fill exactly 1920x1080 (touches all 4 corners)
    
    Args:
        video_path: Path to video file
        output_path: Where to save thumbnail (auto-generated if None)
        time_seconds: Time in seconds to capture frame (default = THUMBNAIL_CAPTURE_TIME)
    
    Returns:
        Path to saved thumbnail image (stretched to fill 1920x1080)
    """
    ...
    if output_path is None:
        output_path = video_path.replace('.mp4', f'_thumbnail_t{time_seconds}.png')
    ...
    try:
        ...
        if len(non_yellow_coords) > 0:
            ...
            print(f"   ✂️ Cropped to content: {cropped_img.size} (removed yellow background)")
            
            # STRETCH to exactly 1920x1080 (fill all 4 corners)
            ...
            # Fallback: stretch full frame if detection fails
            print(f"   ⚠️ Could not detect image content, stretching full frame")
            target_size = (1920, 1080)
...
"""
        ...
        try:
            ...
            # EXTRACT THUMBNAIL FROM VIDEO (at t = THUMBNAIL_CAPTURE_TIME, cropping out yellow background)
            print(f"\n   🖼️ DEBUG: Extracting thumbnail from video at t={THUMBNAIL_CAPTURE_TIME}s...")
            thumbnail_path = extract_thumbnail_from_video(video_path, time_seconds=THUMBNAIL_CAPTURE_TIME)
            ...
            if thumbnail_path and os.path.exists(thumbnail_path):
                try:
                    ...
                    print(f"   🖼️ Thumbnail captured at t={THUMBNAIL_CAPTURE_TIME}s, yellow removed, stretched to 1920x1080")
    ...
    def process_date(self, target_date: str, post_to_youtube: bool = True, 
        ...
        print(f"🎬 Animation: Starts visible at 4.8s, slides up to reveal content")
        print(f"🎬 Thumbnail: Captured at t={THUMBNAIL_CAPTURE_TIME}s, yellow removed, stretched to 1920x1080")
        print("="*60)
...
if __name__ == "__main__":
    ...
    print(f"   🎬 Thumbnail Capture Time: {THUMBNAIL_CAPTURE_TIME} second")
